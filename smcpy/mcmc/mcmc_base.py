...
class BlackJaxMCMC(MCMCBase):
    ...
    def evaluate_model(self, imputs):
    ...
    def _setup_blackjax_sampler(self):
        ...
        if self.sampler_type not in samplers:
            return ValueError(f"Unsupported sampler: {self.sampler_type}. "
    ...
    def _log_posterior_fn(self, params):
        ...
        log_like = self.evlauate_log_likelihood(np_params)

        log_post = self.evlaute_log_posterior(log_like, log_priors)
